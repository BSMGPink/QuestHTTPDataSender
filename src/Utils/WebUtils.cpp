...
{   
	...
	void PostAsync(std::string url, long timeout, std::function<void(long, std::string)> finished, std::function<void(float)> progressUpdate, std::string json) {
		std::thread t (
			[url, timeout, progressUpdate, finished, json] {
				std::string val;
				// Init curl
				auto* curl = curl_easy_init();
				struct curl_slist *headers = NULL;
